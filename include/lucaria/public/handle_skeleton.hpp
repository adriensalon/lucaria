...
private:
    ...
    detail::container_cache<detail::object_skeleton>* _cached = nullptr;
    ...
    void save(ArchiveType& archive) const
    {
        ...
        const uint32 _asset_id = _mappings.objects.skeletons.get(_cached);
    ...
    void load(ArchiveType& archive)
    {
        ...
        detail::container_cache<detail::object_skeleton>* _cached_cell = _mappings.objects.skeletons.get(_asset_id);
        if (_cached_cell == nullptr) {
            LUCARIA_DEBUG_ERROR("Failed to resolve skeleton handle while loading");
            _cached = nullptr;
            _refcount = {};
            return;
        }
