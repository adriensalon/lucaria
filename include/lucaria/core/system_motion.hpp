...
namespace detail {
	...
	struct system_motion {
		...
		void update_advance_controllers(manager_window& window, manager_scene& scenes);
		void update_apply_animations(manager_scene& scenes);
		void update_apply_motion_tracks(manager_window& window, manager_scene& scenes);
