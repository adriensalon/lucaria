...
namespace detail {
    ...
    private:
        ...
        std::vector<rendering_mesh_page> pages = {};
        friend struct rendering_mesh_registry;
    };

    struct rendering_mesh_registry {
        rendering_mesh_registry() = default;
        rendering_mesh_registry(const rendering_mesh_registry&) = delete;
        rendering_mesh_registry& operator=(const rendering_mesh_registry&) = delete;
        ~rendering_mesh_registry();

        [[nodiscard]] rendering_mesh_pool& assure_pool(const data_geometry_profile profile);
        [[nodiscard]] rendering_mesh_pool* find_pool(const data_geometry_profile profile);
    ...
    private:
        std::unordered_map<data_geometry_profile, rendering_mesh_pool> pools = {};
    };
