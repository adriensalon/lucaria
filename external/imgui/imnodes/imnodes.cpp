...
ImNodesContext* GImNodes = NULL;
   	...
   	constexpr auto scan_t = scanf;
#else
   	constexpr auto scan_t = sscanf_s;
#endif
...
void NodeLineHandler(ImNodesEditorContext& editor, const char* const line)
{
    ...
    else if (scan_t(line, "origin=%i,%i", &x, &y) == 2)
    {
        ImNodeData& node = editor.Nodes.Pool[GImNodes->CurrentNodeIdx];
...
{
    (void)scan_t(line, "panning=%f,%f", &editor.Panning.x, &editor.Panning.y);
}
} // namespace
