...
{
    if( !service || !res ) return -1;

    auto ai = (struct addrinfo*)malloc( sizeof( struct addrinfo ) );
    ...
    const auto family = hints ? hints->ai_family : AF_INET;
    ...
    sa->sin_port = htons( (uint16_t)strtoul( service, nullptr, 10 ) );

    if( !node )
    {
        const auto passive = hints && ( hints->ai_flags & AI_PASSIVE );
        sa->sin_addr.s_addr = passive ? htonl( INADDR_ANY ) : htonl( INADDR_LOOPBACK );
    }
    else if( inet_pton( AF_INET, node, &sa->sin_addr ) != 1 )
    {
        auto host = gethostbyname( node );
        if( !host || host->h_addrtype != AF_INET || !host->h_addr_list || !host->h_addr_list[0] )
        {
            free( ai );
            free( sa );
            return -1;
...
static int poll( struct pollfd* fds, unsigned long nfds, int timeout )
{
    ...
    const auto ret = select( fds[0].fd + 1, &readfds, nullptr, nullptr, ptv );
    if( ret > 0 && FD_ISSET( fds[0].fd, &readfds ) )
    {
