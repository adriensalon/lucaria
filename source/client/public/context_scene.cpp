...
{
    return handle_entity { _manager->segment_registry_cpu.create(static_cast<uint16>(_manager->index_for_context)) };
}
...
{
    return _manager->segment_registry_cpu.emplace<component_animator>(entity._entity);
}

component_interface_screen& context_scene::create_screen_interface(const handle_entity entity)
{
    return _manager->segment_registry_cpu.emplace<component_interface_screen>(entity._entity);
}

component_interface_spatial& context_scene::create_spatial_interface(const handle_entity entity)
{
    return _manager->segment_registry_cpu.emplace<component_interface_spatial>(entity._entity);
}

component_model_blockout& context_scene::create_blockout_model(const handle_entity entity)
{
    return _manager->segment_registry_cpu.emplace<component_model_blockout>(entity._entity);
}

component_model_unlit& context_scene::create_unlit_model(const handle_entity entity)
{
    return _manager->segment_registry_cpu.emplace<component_model_unlit>(entity._entity);
}

component_rigidbody_passive& context_scene::create_passive_rigidbody(context_dynamics& dynamics, const handle_entity entity)
{
    return _manager->segment_registry_cpu.emplace<component_rigidbody_passive>(entity._entity, dynamics);
}
