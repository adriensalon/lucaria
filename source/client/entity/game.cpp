...
void game_context::save_snapshot(const std::filesystem::path& path)
{
    ...
    for (detail::scene_implementation& _scene : detail::engine_scenes()) {
        entt::registry* _registry = &_scene.components;
    ...
    for (detail::scene_implementation& _scene : detail::engine_scenes()) {
        const detail::scene_entity_save_database& _entity_ids = detail::engine_scene_database().save_database.scene_entities.at(&_scene.components);
