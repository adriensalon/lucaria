...
namespace detail {
    ...
    void dynamics_system::step_simulation()
    {
        ...
        each_view<passive_rigidbody_component>(entt::exclude<transform_component>, [](passive_rigidbody_component& collider) {
            const bool _collected = collider._shape.has_value(); // colliders can have no transform
            (void)_collected;
        });

        each_view<passive_rigidbody_component, transform_component>([](passive_rigidbody_component& collider, transform_component& transform) {
            if (!collider._shape) {
                return;
            }
        ...
        each_view<kinematic_rigidbody_component, transform_component>([](kinematic_rigidbody_component& rigidbody, transform_component& transform) {
            if (!rigidbody._shape) {
                return;
            }
        ...
        each_view<dynamic_rigidbody_component>([&](dynamic_rigidbody_component& rigidbody) {
            if (!rigidbody._shape) {
                return;
            }
    ...
    void dynamics_system::compute_collisions()
    {
        ...
        each_view<transform_component, kinematic_rigidbody_component>([&](transform_component& transform, kinematic_rigidbody_component& rigidbody) {
...
std::optional<collision> dynamics_context::raycast(const glm::vec3& from, const glm::vec3& to)
{
...
void dynamics_context::set_world_gravity(const glm::float32 newtons)
{
    _dynamics_world->setGravity(convert_bullet(-detail::_world_up * newtons));
}
