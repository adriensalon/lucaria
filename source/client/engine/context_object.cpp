...
handle_cubemap context_object::fetch_cubemap(const std::array<std::filesystem::path, 6>& paths, const std::optional<data_image_profile> profile)
{
    ...
    return fetch_asset<handle_cubemap, detail::object_cubemap>(
        _cache_id,
...
{
    return fetch_asset<handle_event_track, detail::object_event_track>(
        path.string(),
...
{
    return fetch_asset<handle_image, detail::object_image>(
        path.string(),
...
{
    return fetch_asset<handle_mesh, detail::object_mesh>(
        path.string(),
...
{
    return fetch_asset<handle_motion_track, detail::object_motion_track>(
        path.string(),
...
handle_shape context_object::fetch_shape(const std::filesystem::path& path, const detail::object_shape_algorithm algorithm)
{
    ...
    return fetch_asset<handle_shape, detail::object_shape>(
        _cache_id,
...
{
    return fetch_asset<handle_skeleton, detail::object_skeleton>(
        path.string(),
...
{
    return fetch_asset<handle_sound_track, detail::object_sound_track>(
        path.string(),
...
{
    return fetch_asset<handle_texture, detail::object_texture>(
        path.string(),
...
{
    return create_asset<handle_shape, detail::object_shape>(new btCapsuleShape(radius, height), radius + height * 0.5f);
}
