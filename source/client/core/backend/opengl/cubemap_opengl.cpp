...
namespace detail {
        ...
        : origin(images[0].origin == image_origin::path ? cubemap_origin::path : cubemap_origin::data)
    {
        ...
        for (glm::uint _index = 0; _index < 6; ++_index) {
            ...
            case 3:
                if ((_image.data.type == image_data_type::etc2_compressed) && detail::engine_window().is_etc2_supported) {
                ...
                } else if ((_image.data.type == image_data_type::s3tc_compressed) && detail::engine_window().is_s3tc_supported) {
