...
namespace detail {
    ...
    namespace {
        ...
        [[nodiscard]] static rendering_mesh_page _create_geometry_page(const uint32 required_vertex_size, const uint32 required_element_size)
        {
            rendering_mesh_page _page = {};
    ...
    rendering_mesh_registry::~rendering_mesh_registry()
    {
        for (std::pair<const data_geometry_profile, rendering_mesh_pool>& _pair : pools) {
            rendering_mesh_pool& _pool = _pair.second;
            for (rendering_mesh_page& _page : _pool.pages) {
    ...
    void rendering_mesh_registry::upload(rendering_mesh& mesh, const std::vector<uint8>& vertices, const std::vector<uint32>& elements)
    {
        ...
        rendering_mesh_pool& _pool = assure_pool(mesh.profile);
        ...
        if (!_allocation) {
            LUCARIA_DEBUG_ERROR("Failed to allocate rendering mesh storage")
            return;
        }
        rendering_mesh_page& _page = _pool.pages[_allocation->page];
        if (_allocation->vertices.size != 0) {
            glBindBuffer(GL_ARRAY_BUFFER, _page.vertices_id);
            glBufferSubData(
                GL_ARRAY_BUFFER,
                _allocation->vertices.offset,
                _allocation->vertices.size,
                vertices.data());
        }
        if (_allocation->elements.size != 0) {
            ...
            glBufferSubData(
                ...
                _allocation->elements.offset,
                _allocation->elements.size,
                elements.data());
        }
        mesh.geometry_page = _allocation->page;
        mesh.vertices_id = _page.vertices_id;
        mesh.elements_id = _page.elements_id;
        mesh.vertex_offset = _allocation->vertices.offset;
        mesh.vertex_size = _allocation->vertices.size;
        mesh.element_offset = _allocation->elements.offset;
        mesh.element_size = _allocation->elements.size;
    }

    void rendering_mesh_registry::release(rendering_mesh& mesh)
    {
        rendering_mesh_pool* _pool = find_pool(mesh.profile);
        if (_pool == nullptr) {
            return;
        }
        _pool->free({ mesh.geometry_page, { mesh.vertex_offset, mesh.vertex_size }, { mesh.element_offset, mesh.element_size } });
        mesh.geometry_page = rendering_invalid_page;
        mesh.vertex_offset = 0;
        mesh.vertex_size = 0;
        mesh.element_offset = 0;
        mesh.element_size = 0;
        mesh.vertices_id = 0;
        mesh.elements_id = 0;
    }
